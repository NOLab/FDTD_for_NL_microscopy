...
%{
[fpath,fname] =...
    deal('C:\Users\K1343842\Google Drive\MATLAB\chi3-glass-water',...
    'lin-lambda-300-10-2000nm_col-theta-0-5-85deg.txt');
%}
% fpath = 'D:\_FTDT\W201';
% fname = 'W201_water_glass_polar00_width_';
num_det=31;
    ...
    if (B*tt>0)
      ...
      %  if ((pangle==69)==0)
            ...
            for kk=1:nn;
                ...
                THG_int=sum(sum(abs(spec(:,roi)).^2))-sum(sum(abs(spec(:,bck1)).^2))-sum(sum(abs(spec(:,bck2)).^2));%-sum(abs(spec(bck1)).^2)-sum(abs(spec(bck2)).^2);
